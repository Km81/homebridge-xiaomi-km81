...
## 공통 옵션 — `putAway` (임시 연결해제)

계절용 기기 등 **당분간 연결확인이 필요 없는 기기**에 체크하는 기기별 스위치입니다. 5종 모두에서 쓸 수 있습니다.

```jsonc
{ "deviceType": "fan", "name": "선풍기", "ip": "192.168.0.10", "token": "…", "putAway": true }
```

- 켜면 그 기기에 대해 **상태 조회·재연결·클라우드 조회를 전혀 하지 않습니다.** 따라서
  연결이 끊겼다는 기록도 남지 않고, 로그를 읽는 외부 감시도 이 기기를 보지 않게 됩니다.
- **액세서리는 지우지 않습니다.** 홈 앱의 타일·방 배치·장면이 그대로 유지되고,
  타일은 **마지막으로 알고 있던 상태를 그대로 보여 줍니다**(정상 연결로 보입니다).
  설정에서 기기를 아예 삭제하면 타일이 사라져 방 배치와 장면이 깨집니다.
- 타일을 **눌러도 기기에는 전달되지 않습니다.** 값이 잠깐 바뀌었다가 곧 **원래 값으로 돌아갑니다**
  (누른 값이 「마지막 상태」로 저장되지 않게). 공기청정기의 별도 온도·습도·공기질·스위치 타일도 똑같이 동작합니다.
- 이것은 「기기가 죽었다」는 판정이 아니라 **사용자가 선언한 제외**입니다. 기기가 실제로 고장난
  경우에는 켜지 마세요 — 고장을 숨기게 됩니다.
- 설정은 기동 시 한 번만 읽습니다. 켜고 끌 때마다 **홈브릿지를 재시작**해야 반영됩니다.
