...
### MiCloud (클라우드 제어) — 로컬이 불안정한 신형 기기용

`xiaomi.fan.p45`(스마트 타워팬2) 같은 신형 기기는 로컬(LAN) 연결이 자주 끊겨 "응답 없음"이 뜨곤 합니다(앱/클라우드는 정상). 이런 기기는 **클라우드로 제어**할 수 있습니다.

1. 최상위(platform)에 `micloud` 계정 정보를 추가:
   ```jsonc
   "micloud": { "username": "Mi 계정", "password": "비밀번호", "country": "sg" }
   ```
   - `country`: 기기가 등록된 Mi Home 지역. 한국 사용자는 보통 `cn` 또는 `sg`.
2. 해당 선풍기 항목에 `"forceMiCloud": true` 와 `model`·`deviceId` 지정.
