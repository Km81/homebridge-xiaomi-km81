...
const { sleep, requireValidIpAndToken } = require('../common/helpers.js');
  ...
  startPollLoop() {
    ...
    const tick = async () => {
      try {
        ...
        if (!this._shutdown) {
          this.pollTimer = setTimeout(tick, this.cfg.pollingMs || this.cfg.pollingInterval);
        }
      }
    };
  ...
  async poll(isBurst = false) {
    ...
    try {
      if (this.mode === 'legacy') {
        ...
        // auto: miot 먼저 시도, 미지원 응답이면 legacy로 폴백
        try {
          await this.miotPoll(isBurst);
          ...
          if (/get_properties.*not supported|Method.*not supported|-32601/i.test(msg)) {
            this.mode = 'legacy';
            this.logWarn('MIoT 미지원 → Legacy 모드 전환');
            await this.legacyPoll(isBurst);
          } else {
            throw e;
          }
