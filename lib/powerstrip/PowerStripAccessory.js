...
class PowerStripAccessory {
  ...
  setupSwitchService() {
    ...
    this.switchService.getCharacteristic(Characteristic.On).onSet(async (value) => {
      const boolVal = !!value;
      // v2.2.2 — 성공 명령이 기본 레벨에서 안 보이던 것 해소 (실패 시엔 아래 logError 가 짝으로 남는다)
      this.logInfo(`전원 → ${boolVal ? '켜짐' : '꺼짐'}`);
      try {
        // 1) 낙관적 UI 업데이트
  ...
  async connect() {
    ...
    try {
      ...
      this.device = await withTimeout(
        new LocalMiioDevice(this.cfg.ip, this.cfg.token, null, this.cfg.model, this.log).connect(),
      ...
      this.logInfo(`연결됨 (${this.cfg.model || this.mode})`);
      this.connectingAttempt = 0;
      this.consecutiveFailures = 0;
