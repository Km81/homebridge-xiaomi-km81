...
// === MIIO 응답 헬퍼: get_prop 응답 배열을 prop 객체로 변환 ===
function miioGetPropArgs(_propsList) { /* 기본: prop name이 그대로 args */ return null; }
...
const MODELS = {
  ...
  'deerma.humidifier.jsq4': {
    ...
    // water_level: 0 = no water, otherwise OK
    waterLevel: { key: 'water_level', mapFn: (v) => v === 0 ? 0 : 100 },
    buzzer:    { key: 'buzzer', on: true, off: false },
    led: { key: 'switch_status', levels: [false, true], on: true, off: false },
  },
