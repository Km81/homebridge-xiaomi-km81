...
const { clamp, isFiniteNumber, sleep, applyServiceName, requireValidIpAndToken } = require('../common/helpers.js');
...
class HumidifierAccessory {
  ...
  normalizeConfig(c) {
    ...
    return {
      ...
      // sub-services
      enableTemperatureSensor: !!c.enableTemperatureSensor,
      temperatureSensorName: c.temperatureSensorName,
      enableHumiditySensor: !!c.enableHumiditySensor,
  ...
  async connect() {
    ...
    try {
      ...
      this.device = await miio.device({ address: this.cfg.ip, token: this.cfg.token, model: this.cfg.model });
      if (typeof this.device.init === 'function') { try { await this.device.init(); } catch (_) {} }
  ...
  async miotGetAll() {
    ...
    const res = await this.device.call('get_properties', params);
    const out = {};
  ...
  async miioGetAll() {
    ...
    const out = {};
    if (md.getArgsEmpty) {
      const res = await this.device.call(getCall, []);
      if (Array.isArray(res)) {
      ...
      for (let i = 0; i < keys.length; i += batch) {
        ...
        const res = await this.device.call(getCall, slice);
        if (Array.isArray(res)) {
  ...
  async callSet(propKey, value, miioCall) {
    ...
    if (this.protocol === 'miot') {
      ...
      const res = await this.device.call('set_properties', [{ did: propKey, ...def, value }]);
      if (Array.isArray(res)) {
        const r = res[0];
    ...
    const res = await this.device.call(callName, [value]);
    // miio set 결과는 ['ok']
