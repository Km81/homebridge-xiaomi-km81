...
class FanAccessory {
  ...
  constructor(ctx, config) {
    ...
    // 캐시 액세서리가 있으면 즉시 참조. 없으면 FAN_DEVICE_READY 이벤트 시 등록한다.
    // (즉시 등록하면 디바이스 연결 실패 시 services 없는 빈 액세서리가 남아 HomeKit 에서 안 보이거나
    //  혼란을 일으킴. 기존 homebridge-xiaomi-fan-km81 의 lazy-register 패턴을 따른다.)
  ...
  /**
   * FAN_DEVICE_READY 이벤트 시 호출되어 액세서리가 아직 등록 안 된 경우 등록한다.
   */
  ensureAccessoryRegistered() {
    if (this.fanAccesory) return;
  ...
  discoverFan() {
    ...
    this.fanController.on(Events.FAN_DEVICE_READY, (fanDevice) => {
      ...
      // ready 이벤트 시점에 (필요하면) 액세서리를 등록하고 service 를 설정한다.
      this.ensureAccessoryRegistered();
      if (!this.servicesInitialized && this.fanDevice && this.fanAccesory) {
  ...
  async getPowerState() {
    const { Characteristic } = this;
    let on = false;
    if (this.fanDevice && this.fanDevice.isFanConnected()) on = this.fanDevice.isPowerOn();
  ...
  async getFanState() {
    const { Characteristic } = this;
    if (this.fanDevice && this.fanDevice.isFanConnected()) {
      return this.fanDevice.isPowerOn() ? Characteristic.CurrentFanState.BLOWING_AIR : Characteristic.CurrentFanState.IDLE;
    }
  ...
  updateFanStatus() {
    ...
    if (this.fanService) {
      this.fanService.getCharacteristic(Characteristic.Active).updateValue(
        this.fanDevice.isPowerOn() ? Characteristic.Active.ACTIVE : Characteristic.Active.INACTIVE
      );
      if (this.fanDevice.supportsFanSpeed()) {
        this.fanService.getCharacteristic(Characteristic.RotationSpeed)
