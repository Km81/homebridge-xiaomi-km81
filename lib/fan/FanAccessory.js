...
const { isPutAway, PUT_AWAY_SEAL_EMPTY, sealForPutAway } = require('../common/putAway.js');

const BATTERY_LOW_THRESHOLD = 20;
const BUTTON_RESET_TIMEOUT = 20;
const FAN_PLATFORM_SUBKEY = 'xiaomifan';
// 전원 set 직후 보호 구간(ms). 끄기/켜기 직후 폴링이 아직 전이가 끝나지 않은
// stale 값을 읽어 Active 가 잠깐 반대로 깜빡이는 것을 막는다. (전 장비 4초 통일)
const POWER_GRACE_MS = 4000;
...
class FanAccessory {
  ...
  constructor(ctx, config) {
    ...
    // ★임시 연결해제 — 통신을 일절 하지 않는다(availability 가 아니라 사람의 선언). lib/common/putAway.js
    this.putAway               = isPutAway(config);
    this.forceMiCloud          = config.forceMiCloud === true;
    this.cloudFallback         = config.cloudFallback === true;
    this.miCloudCountry        = (typeof config.miCloudCountry === 'string' && config.miCloudCountry.trim()) ? config.miCloudCountry.trim().toLowerCase() : null;
  ...
  shutdown() {
    if (this.rotationSpeedTimeout) { clearTimeout(this.rotationSpeedTimeout); this.rotationSpeedTimeout = null; }
    if (this.angleLightbulbTimeout) { clearTimeout(this.angleLightbulbTimeout); this.angleLightbulbTimeout = null; }
    this.powerGrace = null;
  ...
  discoverFan() {
    ...
    this.fanController.putAway = this.putAway;
    // MiCloud(클라우드) 모드 전달. 세 가지 모드:
    //  - forceMiCloud:  클라우드 전용
    //  - cloudFallback: 로컬 우선 + 실패 시 클라우드 자동 전환(하이브리드)
    //  - (둘 다 꺼짐): 로컬 전용
    // miCloudCountry: 기기가 등록된 지역이 계정 기본과 다를 때(예: 일부 tw / 일부 cn) 호출별 덮어쓰기.
    this.fanController.miCloud = (this.ctx && this.ctx.miCloud) || null;
    this.fanController.forceMiCloud = this.forceMiCloud && !!this.fanController.miCloud;
    this.fanController.cloudFallback = this.cloudFallback && !!this.fanController.miCloud;
    this.fanController.miCloudCountry = this.miCloudCountry;
    // 클라우드 모드는 deviceId 가 필수다(클라우드는 로컬 핸드셰이크로 did 를 알아낼 수 없음).
    // 없으면 클라우드를 끄고 로컬로 동작시킨다(로컬은 핸드셰이크로 did 를 학습할 수 있음).
    if ((this.fanController.forceMiCloud || this.fanController.cloudFallback) && !this.deviceId && !this.putAway) {
      this.logError(`'${this.name}': 클라우드 제어에는 deviceId 가 필요합니다. deviceId 를 지정하세요. 지금은 클라우드를 끄고 로컬로 동작합니다.`);
      this.fanController.forceMiCloud = false;
      this.fanController.cloudFallback = false;
    }
    ...
    this.fanController.on(Events.FAN_DEVICE_READY, (fanDevice) => {
      ...
      try {
        ...
        // ★임시 연결해제면 마지막 상태를 정상 연결로 보여 준다(서비스를 만든 **뒤**여야 한다).
        if (this.putAway && sealForPutAway(this.fanAccesory) === 0) this.logWarn(PUT_AWAY_SEAL_EMPTY);
  ...
  prepareAngleButtonsService() {
    ...
    // 전구(슬라이더)를 쓰면 각도 스위치는 만들지 않는다. 단, 사용자가 angleButtons 를
    // 명시적으로 지정한 경우엔 둘 다 원하는 것으로 보고 그대로 만든다.
    if (this.angleLightbulb === true && (this.angleButtons === undefined || this.angleButtons === null)) return;
  ...
  prepareAngleLightbulbService() {
    ...
    if (Array.isArray(levels) && levels.length > 0) {
      ...
      // 모델 기본값과 동일 규칙: HomeKit 밝기는 0~100 이므로 100°를 넘는 단계가
      // 여러 개면 가장 큰 값 하나만 사용한다 (예: [30,60,90,120,150] → [30,60,90,150]).
  ...
  applyPowerGrace(actualOn) {
    ...
    // 실제 상태가 목표에 도달했거나 보호 시간이 지나면 즉시 해제한다.
    // (물리 버튼/리모컨으로 바꾼 실제 상태를 너무 오래 가리지 않도록)
    if (actualOn === this.powerGrace.target || Date.now() >= this.powerGrace.expire) {
      this.powerGrace = null;
      return actualOn;
    }
  ...
  async setRotationSpeed(value) {
    if (this.fanDevice && this.fanDevice.isFanConnected()) {
      ...
      this.rotationSpeedTimeout = setTimeout(() => {
        if (this.fanDevice && this.fanDevice.isFanConnected()) this.fanDevice.setRotationSpeed(value);
      }, 500);
    } else {
      throw new this.hap.HapStatusError(this.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE);
    }
  ...
  checkAngleButtonValue(v) {
    if (this.fanDevice.supportsOscillationAngle()) {
      if (!this.fanDevice.checkOscillationAngleWithinRange(v)) {
        this.logWarn(`회전 각도 ${v}° — 지원 범위 밖 ${JSON.stringify(this.fanDevice.oscillationAngleRange())}`);
        return false;
      }
    } else if (this.fanDevice.supportsOscillationLevels()) {
      if (!this.fanDevice.checkOscillationLevelSupported(v)) {
        this.logWarn(`회전 각도 ${v}° — 지원 단계 아님 ${JSON.stringify(this.fanDevice.oscillationLevels())}`);
        return false;
      }
    }
    return true;
