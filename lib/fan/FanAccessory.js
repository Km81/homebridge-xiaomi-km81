...
class FanAccessory {
  ...
  constructor(ctx, config) {
    ...
    // 캐시 액세서리 재사용 or 신규 생성
    let accessory = this.ctx.accessories.get(this.UUID);
    if (!accessory) {
      accessory = new this.api.platformAccessory(this.name, this.UUID, this.hap.Categories.FAN);
      ...
      this.logDebug(`캐시 액세서리 재사용: ${accessory.displayName}`);
    }
    this.fanAccesory = accessory;
