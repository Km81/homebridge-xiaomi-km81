...
class MiotSmartmiDcFan extends MiotFan {
  ...
  initFanCapabilities() {
    ...
    // za5 의 6,2(battery-state)·6,5(ac-state)는 "배터리/AC 유무" bool 일 뿐 배터리 잔량이
    // 아니다(스펙상 잔량 속성 자체가 없음). BUILT_IN_BATTERY 를 켜면 getBatteryLevel()이
    // 0 을 반환해 홈킷이 영구 0%·배터리 부족 경고를 띄우므로 노출하지 않는다(reference 동일).
  }
  ...
  getSpeed() { return this.getSafePropertyValue(this.getFanProperties().fan_speed_rpm, 0); }
  ...
  isLedEnabled() { return this.getLedBrightness() > 0; }
  getLedBrightness() { return this.getFanProperties().light; }
  ...
  isIoniserEnabled() { return this.getFanProperties().anion === true; }
  getTemperature() { return this.getFanProperties().temperature; }
  getRelativeHumidity() { return this.getFanProperties().relative_humidity; }
  ...
  async moveLeft()                   { return this.sendCommnd('set_move', 'left'); }
  async moveRight()                  { return this.sendCommnd('set_move', 'right'); }
  async setBuzzerEnabled(enabled)    { return this.setProperty('buzzer', enabled); }
  async setLedEnabled(enabled)       { return this.setProperty('light', enabled ? 100 : 0); }
  ...
  async setIoniserEnabled(enabled)   { return this.setProperty('anion', enabled); }
}
