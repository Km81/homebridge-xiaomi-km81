...
const { fmtMiot } = require('../../../common/CommandLog.js');
...
class MiotFan extends BaseFan {
  ...
  modelSpecificSetup() {
    ...
    this.commandDefs = {};
    if (!this.deviceId) {
      this.logError(`deviceId를 찾지 못했습니다 — 클라우드/miot 제어에는 deviceId가 필수입니다.`);
    }
  }
  ...
  defineProperty(prop, siid, piid) {
    if (!prop || !siid || !piid) {
      this.logWarn(`속성 추가 불가 (prop=${prop}, siid=${siid}, piid=${piid})`);
      return;
    }
  ...
  defineCommand(cmd, siid, piid) {
    if (!cmd || !siid || !piid) {
      this.logWarn(`명령 추가 불가 (cmd=${cmd}, siid=${siid}, piid=${piid})`);
      return;
    }
  ...
  // 모델별 모드 값 라벨 훅 — 기기 클래스가 오버라이드해 한국어 라벨을 제공하면
  // CommandLog 가 `모드 → <라벨>` info 를 남긴다. 기본은 null(모드는 debug 만).
  // (mode 값의 의미가 모델마다 달라 전역 사전으로는 오표기 위험이 있기 때문)
  _miotValueLabel(prop, value) { return null; } // eslint-disable-line no-unused-vars

  // 성공 명령 info 한 줄 (v2.2.2). 입구에서 남겨 로컬·클라우드 양 경로를 모두 커버.
  _logCmdInfo(prop, value) {
    const label = fmtMiot(prop, value, this._miotValueLabel(prop, value));
    if (label) this.logInfo(label);
  }

  async sendCommnd(cmd, value) {
    this._logCmdInfo(cmd, value);
    ...
    if (this.miioFanDevice) {
      ...
      return withTimeout(this.miioFanDevice.call('set_properties', [cmdDef]), CALL_TIMEOUT_MS, `cmd ${cmd}`).then(result => {
        ...
        this.logWarn(`명령 실행 실패 (${cmd}=${value}): ${err}`);
      });
    }
  ...
  async setProperty(prop, value) {
    ...
    if (this.isFanConnected()) {
      ...
      return withTimeout(this.miioFanDevice.call('set_properties', [propDef]), CALL_TIMEOUT_MS, `set ${prop}`).then(result => {
        ...
        this.logWarn(`속성 설정 실패 (${prop}=${value}): ${err}`);
      });
    }
  ...
  async requestAllPropertiesCloud() {
    ...
    try {
      ...
      if (err && err.authFailed && !this._cloudAuthWarned) {
        ...
        // v2.2.1 — 자동 재로그인 가동 중엔 "수동 로그인" 안내가 오도 → 자동 복구 대기 안내로 분기
        if (err.autoRelogin) this.logWarn(`MiCloud 세션 만료 — 자동 재로그인 진행/대기 중입니다 (자동 복구 예정): ${err.message}`);
        else this.logWarn(`MiCloud 세션이 만료되었거나 인증에 실패했습니다 — 설정 화면(또는 tools/micloud-login.js)에서 다시 로그인하세요.`);
      }
      throw err;
    }
    if (this._cloudAuthWarned) this.logInfo('(cloud) MiCloud 인증 복구 확인 — 클라우드 폴링 재개'); // v2.0.0
    ...
    if (flat.length === 0) {
      ...
      if (this._cloudEmptyWarned) this.logInfo('(cloud) 기기 데이터 수신 재개'); // v2.0.0
