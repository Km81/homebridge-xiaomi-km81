...
class XiaomiFanP45 extends MiotFan {
  ...
  initFanCapabilities() {
    ...
    this.addCapability(FanCapabilities.FAN_SPEED_CONTROL, true);   // HomeKit 속도 슬라이더 (기어 1~4 → 25% 단위)
  ...
  addFanProperties() {
    ...
    this.defineProperty('fan_level', 2, 4);         // 단계 풍속 1-4 (HomeKit 속도 25% 단위 매핑)
  ...
  /*
   * ──────────── 풍속 단계 매핑 ────────────
   * 기존 Smartmi 선풍기(v3/za4)와 동일하게 HomeKit 회전속도 슬라이더를
   * 기어 단계 1~4 와 25/50/75/100% 로 양방향 매핑한다. p45 는 기어(fan-level
   * 2,4)와 모드(직풍/자연풍/수면)가 독립이라, 모드와 무관하게 항상 기어 단계를
   * 슬라이더에 반영한다.
  ...
  async setRotationSpeed(speed) {
    ...
    if (stage < 1) stage = 1;            // fan-level 유효범위 1~4 (0 은 무효)
