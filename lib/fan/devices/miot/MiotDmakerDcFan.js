...
class MiotDmakerDcFan extends MiotFan {
  ...
  addFanProperties() {
    ...
    // 일부 모델(p11/p15/p33)은 무단계 속도 속성이 없어 4단 기어를 회전속도 슬라이더에 매핑한다.
    this._speedViaLevel = false;
    ...
    if (fanModel === 'dmaker.fan.p9') {
      ...
      // p11, p15, p33: 2,6 은 read-only fan:status 라 무단계 속도 속성이 없다.
      // 4단 기어(fan_level)를 회전속도 슬라이더에 매핑한다(레퍼런스 동작과 동일).
      this._speedViaLevel = true;
      this.defineProperty('child_lock', 7, 1);
      this.defineProperty('swing_mode', 2, 4);
      this.defineProperty('swing_mode_angle', 2, 5);
      this.defineProperty('power_off_time', 3, 1);
  ...
  // 4단 기어 <-> HomeKit % 양방향 매핑 (1→25, 2→50, 3→75, 4→100)
  ...
  getRotationSpeed() {
    if (this._speedViaLevel) return this._stageToPercent(this.getSafePropertyValue(this.getFanProperties().fan_level, 0));
  ...
  async setRotationSpeed(speed) {
    if (this._speedViaLevel) {
      let stage = this._percentToStage(speed);
      if (stage < 1) stage = 1;   // 1~4 기어만 유효
  ...
  async setAngle(angle) {
    // 이 팬들은 미리 정의된 각도만 지원한다. 상한을 모델이 지원하는 최대 레벨로 동적 적용
    // (예: dmaker.fan.p9 는 150°까지 지원 → 140°로 깎이지 않게 함).
    const levels = this.oscillationLevels();
    const maxAngle = (Array.isArray(levels) && levels.length > 0) ? Math.max(...levels) : 140;
