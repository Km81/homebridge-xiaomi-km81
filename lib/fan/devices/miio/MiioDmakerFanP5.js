...
class MiioDmakerFanP5 extends MiioFan {
  ...
  getRotationSpeed() { return this.getSafePropertyValue(this.getFanProperties().speed, 0); }
  isChildLockActive() { return this.getFanProperties().child_lock === true; }
  isSwingModeEnabled() { return this.getFanProperties().roll_enable === true; }
  getAngle() { return this.getFanProperties().roll_angle; }
  isNaturalModeEnabled() { return this.getFanProperties().mode === 'nature'; }
  isBuzzerEnabled() { return this.getFanProperties().beep_sound === true; }
  isLedEnabled() { return this.getFanProperties().light === true; }
  getShutdownTimer() { return this.getFanProperties().time_off; }
  ...
  async setNaturalModeEnabled(enabled) {
    const mode = enabled ? 'nature' : 'normal';
    this.updateProperty('mode', mode);
    return this.sendCommand('s_mode', mode, ['mode']);
  }

  async moveLeft()  { return this.sendCommand('m_roll', 'left'); }
  async moveRight() { return this.sendCommand('m_roll', 'right'); }
