...
const { fmtMiio } = require('../../../common/CommandLog.js');
...
class MiioFan extends BaseFan {
  ...
  // opts.quietInfo: 상위(의미) 계층이 이미 info 를 남긴 경우(자연풍 매핑 등) 같은 동작에
  //                 두 줄이 찍히지 않도록 여기서는 debug 만 남긴다.
  async sendCommand(cmd, value, refresh, refreshDelay = 200, opts) {
    if (this.isFanConnected()) {
      // v2.2.2 — 성공한 명령이 기본 레벨에서 안 보이던 것 해소: 전송 시점에 한국어 한 줄 info.
      // 매핑이 없는 명령은 null → 기존처럼 debug 만 (영문 식별자를 info 에 노출하지 않음).
      if (!(opts && opts.quietInfo)) {
        const label = fmtMiio(cmd, value);
