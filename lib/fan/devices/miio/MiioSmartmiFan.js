...
class MiioSmartmiFan extends MiioFan {
  ...
  async setRotationSpeed(speed) {
    ...
    // 자연풍은 위에서 이미 매핑 상세와 함께 info 를 남겼다 → 전송 계층 중복 억제(quietInfo).
    // 일반풍은 전송 계층이 `풍량 → N%` 한 줄을 남긴다 (dedup 으로 전송이 생략되면 로그도 없음 = 정확).
    return this.sendCommand(setMethod, targetSpeed, ['speed_level', 'natural_level'], SET_REFRESH_DELAY_MS, { quietInfo: isNatural });
  }
  ...
  async setNaturalModeEnabled(enabled) {
    ...
    // 위에서 `자연풍 켜기/끄기 → …` info 를 이미 남겼다 → 전송 계층 중복 억제.
    return this.sendCommand(setMethod, targetSpeed, ['speed_level', 'natural_level'], SET_REFRESH_DELAY_MS, { quietInfo: true });
  }
