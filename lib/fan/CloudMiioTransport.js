  ...
  async _rawCall(method, params) {
    ...
    try {
      ...
      if (err && err.authFailed && !this._authWarned) {
        ...
        if (err.autoRelogin) this._warn(`MiCloud 세션 만료 — 자동 재로그인 진행/대기 중입니다 (자동 복구 예정): ${err.message}`);
        else this._warn(`MiCloud 세션이 만료되었거나 인증에 실패했습니다 — 설정 화면(또는 tools/micloud-login.js)에서 다시 로그인하세요.`);
      }
      throw err;
    }
