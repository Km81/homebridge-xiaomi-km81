...
class LegacyMiioTransport {
  ...
  _applyValues(names, values) {
    ...
    // get_prop 응답이 요청보다 짧으면(펌웨어가 뒤 속성을 잘라 보냄) 뒤쪽 속성이 옛 값으로
    // 조용히 굳는다. 값은 안전하게 옛 값을 유지하되, 진단 가능하도록 debug 로 남긴다.
    if (values.length < names.length) {
      try { this.log.debug && this.log.debug(`(miio) get_prop 응답이 ${values.length}/${names.length}개로 짧음 — 뒤 속성 이전 값 유지: ${names.slice(values.length).join(',')}`); } catch (_) {}
    }
