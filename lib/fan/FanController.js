...
const { isPutAway, PUT_AWAY_MESSAGE } = require('../common/putAway.js');
...
// 백스톱: 로컬 재탐색이 이만큼(≈2h, 5분 간격 × 24회) 연속 실패하면 컨트롤러를 전체 재초기화한다.
// fresh socket 으로도 안 풀리는 미지의 wedge 에 대한 마지막 수단(예전엔 프로세스 재시작만이 복구였다).
const CLOUD_STUCK_RESET_AFTER_FAILS = 24;
...
class FanController extends EventEmitter {
  constructor(ip, token, deviceId, model, name, pollingInterval, log) {
    ...
    if (!this.ip) this.logError(`설정에 ip가 없습니다.`);
    if (!this.token) this.logError(`설정에 token이 없습니다.`);
  ...
  connectToFan() {
    // ⛔★★임시 연결해제: 어떤 통신도 시작하지 않는다(v2.3.0).
    //   여기가 **유일한 진입점**이라 여기서 막으면 로컬·클라우드·재탐색이 모두 안 열린다.
    //   ⚠️그래도 startPutAway() 가 _shutdown 을 세워 이중으로 봉한다 —
    //     나중에 누가 다른 경로에서 startFanDiscovery() 를 부르더라도 부활하지 못하게.
    if (this.putAway) { this.startPutAway(); return; }
  ...
  // 임시 연결해제(putAway): 통신을 일절 하지 않는다. 서비스만 구성해 홈킷 타일을 살려 둔다.
  //   ⛔액세서리를 지우면 방 배치·장면이 깨진다(브릿지 재페어링 금지와 같은 부류) — 그래서 남긴다.
  //   ★_shutdown 을 세우는 것이 핵심이다. 모든 통신 진입점(startFanDiscovery / startFanPolling /
  //     startCloudPollLoop / probeLocal / tripToCloud / _hardReset)이 이 표식을 보고 되돌아간다.
  startPutAway() {
    this.logInfo(PUT_AWAY_MESSAGE);
    this._shutdown = true;
    this._stopPolling = true;
    // model 이 있으면 연결 없이 디바이스 객체만 만들어 FAN_DEVICE_READY 를 올린다
    // (FanAccessory 가 이 이벤트에서 Fanv2/스위치 서비스를 붙인다 — FanAccessory 가 곧바로 sealForPutAway 로 처리기를 떼어 **마지막 상태**로 답하게 한다).
    // ⚠️model 이 없으면(자동탐지 설정) 만들 수 없다. 캐시 액세서리는 옛 서비스를 그대로 쓰고,
    //   처음 등록되는 기기라면 타일에 기본 정보만 보인다 — 임시 연결해제한 기기이므로 실해는 없다.
    if (this.model && this.model.length > 0) this.createFanDevice(null, this.model);
  }
  ...
  tripToCloud() {
    ...
    this._cloudProbeFails = 0;
    this._cloudPollOk = false;   // 이번 클라우드 세션이 실제로 폴링에 성공했는지 (백스톱 게이트용)
  ...
  startCloudPollLoop() {
    ...
    const tick = () => {
      ...
      this.withTimeout(this.fanDevice.pollProperties(), POLL_TIMEOUT_MS, 'cloud poll')
        .then(result => { this._cloudPollOk = true; this.emit(Events.FAN_PROPERTIES_UPDATED, result); })
        .catch(err => { this._cloudPollOk = false; this.logDebug(`Cloud poll failed (로그인 전이거나 일시 오류일 수 있음): ${err}`); })
  ...
  async probeLocal() {
    ...
    try {
      ...
      // 평소엔 debug. 단, 로컬이 오래 안 잡혀 클라우드에 계속 머무르면(예전엔 debug 라
      // 'stuck' 이 로그에 안 보였다) 5회(≈25분)마다 한 번 보이게 경고해 진단을 돕는다.
      this._cloudProbeFails = (this._cloudProbeFails || 0) + 1;
      // 백스톱: 로컬이 오래(≈2h) 안 잡히고 + 클라우드마저 실패할 때(= 기기가 진짜 제어 불가)만
      // 전체 재초기화한다. 클라우드로는 정상 제어되는 경우(로컬만 막힌 흔한 상황)엔 재초기화하지
      // 않는다 — probe 는 이미 매번 fresh socket 이라 로컬에 추가 이점이 없고, 멀쩡한 클라우드
      // 세션을 버리면 오히려 로컬 재시도 동안 ~15~40초 끊김이 주기적으로 생긴다.
      if (this._cloudProbeFails >= CLOUD_STUCK_RESET_AFTER_FAILS && !this._cloudPollOk) {
        const mins = Math.round((Date.now() - this._cloudSince) / 60000);
        this.logWarn(`로컬·클라우드 모두 실패(약 ${mins}분, 로컬 재탐색 ${this._cloudProbeFails}회) — 컨트롤러를 전체 재초기화해 마지막 복구를 시도합니다.`);
        this._hardReset();
        return;
      }
      if (this._cloudProbeFails % 5 === 0) {
        const mins = Math.round((Date.now() - this._cloudSince) / 60000);
        this.logWarn(`로컬 재탐색 ${this._cloudProbeFails}회 실패 — ${mins}분째 클라우드로 동작 중입니다. 로컬이 계속 안 잡히면 기기 전원을 리셋해 보세요. (${(e && e.message) || e})`);
      } else {
  ...
  // 백스톱: 클라우드에 너무 오래 갇혀 로컬 재탐색이 계속 실패하면, 컨트롤러 상태를 전부 정리하고
  // 로컬 연결부터 처음처럼 다시 시도한다. fanDevice 객체 자체는 유지(HomeKit 서비스 바인딩 보존)하고
  // 전송만 새로 꽂는다. 로컬이 여전히 안 되면 다시 클라우드로 폴백하며 카운터가 리셋되어 ≈2h 뒤 재시도.
  _hardReset() {
    if (this._shutdown) return;
    this._stopPolling = true;
    this._pollGen++;
    ...
    if (this._retryTimer) { clearTimeout(this._retryTimer); this._retryTimer = undefined; }
    if (this._localProbeTimer) { clearTimeout(this._localProbeTimer); this._localProbeTimer = undefined; }
    if (this.fanDevice && typeof this.fanDevice.teardownCloud === 'function') {
      try { this.fanDevice.teardownCloud(); } catch (_) {}
    }
    if (this.fanDevice) { try { this.fanDevice.disconnectAndDestroyMiioDevice(); } catch (_) {} }
    this._mode = 'local';
    this._reconnectAttempt = 0;
    this._cloudProbeFails = 0;
    this._connecting = false;
  ...
  withTimeout(promise, ms, label) {
    return new Promise((resolve, reject) => {
      let settled = false;
      const t = setTimeout(() => { settled = true; reject(new Error(`${label} timed out after ${ms}ms`)); }, ms);
      promise.then(
        (v) => {
          // 타임아웃(또는 이미 settle) 후 뒤늦게 resolve 되면, 그 값이 살아있는 연결(device/transport)
          // 이면 파기한다. 안 그러면 outer 타임아웃이 inner 핸드셰이크보다 먼저 발화하거나 자동탐지
          // (aholstenson miio.device) 경로에서 소켓이 누수된다. 속성 객체 등 destroy 가 없는 값은 무시.
          if (settled) { try { v && typeof v.destroy === 'function' && v.destroy(); } catch (_) {} return; }
          clearTimeout(t); settled = true; resolve(v);
        },
        (e) => { if (settled) return; clearTimeout(t); settled = true; reject(e); }
      );
    });
  }
  ...
  startFanDiscovery() {
    ...
    this.withTimeout(this._openConnection(), CONNECT_TIMEOUT_MS, 'connect')
      .then(device => {
        ...
        if (this._retryTimer) { clearTimeout(this._retryTimer); this._retryTimer = undefined; }
        this.logInfo(`연결됨 (${device.miioModel})`);
