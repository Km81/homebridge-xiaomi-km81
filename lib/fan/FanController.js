...
const CONNECT_TIMEOUT_MS = 12000;   // miio.device() 핸드셰이크 최대 대기
const POLL_TIMEOUT_MS = 10000;      // 단일 폴링 최대 대기
...
class FanController extends EventEmitter {
  ...
  startFanDiscovery() {
    ...
    this.withTimeout(miio.device({ address: this.ip, token: this.token }), CONNECT_TIMEOUT_MS, 'connect')
      ...
      .catch(err => {
        this._connecting = false;
        this.logDebug(`Could not connect to the fan! Retrying in ${checkDelayTime / 1000} seconds! Error: ${err}`);
