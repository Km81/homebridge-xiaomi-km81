...
class FanController extends EventEmitter {
  ...
  // 클라우드 폴백/전용을 쓸 수 있는 조건: 세션·모델이 있고, 모델이 miot(클라우드 제어 가능)여야 한다.
  // 레거시 miio 선풍기(zhimi.fan.v3/za4 등)는 MiioFan 이 클라우드 경로를 구현하지 않으므로 제외한다.
  _canUseCloud() { return !!(this.miCloud && this.model && this.model.length > 0 && FanDeviceFactory.isMiotModel(this.model)); }
  ...
  connectToFan() {
    // 레거시 miio 선풍기는 클라우드를 지원하지 않는다. forceMiCloud/cloudFallback 이 켜져 있어도
    // 무시하고 로컬로만 동작시킨다(켜져 있으면 한 번 경고 — 잘못된 기대를 막기 위해).
    if ((this.forceMiCloud || this.cloudFallback) && this.miCloud && this.model && !FanDeviceFactory.isMiotModel(this.model)) {
      this.logWarn(`'${this.model}' 은(는) 레거시 miio 기기라 클라우드(forceMiCloud/하이브리드)를 지원하지 않습니다. 로컬로만 동작합니다.`);
      this.forceMiCloud = false;
