...
class FanController extends EventEmitter {
  ...
  async probeLocal() {
    ...
    try {
      ...
      // 백스톱: 너무 오래(≈2h) 갇혀 있으면 전체 재초기화로 마지막 복구를 시도한다.
      if (this._cloudProbeFails >= CLOUD_STUCK_RESET_AFTER_FAILS) {
        const mins = Math.round((Date.now() - this._cloudSince) / 60000);
        this.logWarn(`로컬 재탐색 ${this._cloudProbeFails}회 연속 실패(약 ${mins}분) — 컨트롤러를 전체 재초기화해 마지막 복구를 시도합니다.`);
        this._hardReset();
        return;
      }
