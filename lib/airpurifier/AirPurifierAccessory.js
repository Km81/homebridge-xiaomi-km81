...
const { fmtPurifier, purifierModeLabel } = require('../common/CommandLog.js');
...
class AirPurifierAccessory {
  constructor(ctx, config) {
    ...
    if (isPutAway(config)) {
      ...
      // ★마지막 상태를 정상 연결로 표시. ⚠️자식 타일(별도 온도·습도·AQI·스위치 액세서리)도 함께 —
      //   본체만 봉하면 자식 스위치 탭이 `updateAll()` 로 **본체 타일을 0 으로 덮었다**(2026-09-15 리뷰).
      const owned = [this.accessory, ...Object.values(this.child).map(c => c && c.acc)];
      if (sealForPutAway(owned) === 0) this.logWarn(PUT_AWAY_SEAL_EMPTY);
      this._shutdown = true;   // 이중 잠금 — 재연결·폴링 경로가 전부 이 표식을 본다
      return;
    }
  ...
  setupAirPurifierService() {
    ...
    ap.getCharacteristic(Characteristic.TargetAirPurifierState).onSet(async (v) => {
      ...
      if (wantsAuto && this.config.keepFavoriteMode
        ...
        this.logInfo('keepFavoriteMode: 전원 켜기에 동반된 자동 모드 요청 무시 (수동 유지)');
        setTimeout(() => {
          try {
    ...
    try {
      ...
      while (attempt < CONNECT_RETRY_MAX && !this.device) {
        ...
        try {
          ...
          this.device = device;
          this.logInfo(`연결됨 (${device.miioModel || this.config.type || 'AirPurifier'})`);
          return true;
        } catch (e) {
  ...
  async maybeEnforceFavorite() {
    try {
      ...
      this.logInfo(`keepFavoriteMode: 외부에서 모드가 '${purifierModeLabel(m) || m}'로 바뀐 것을 감지 → 수동 복귀`);
      this.beginGrace('mode', 'favorite');
      this.state.mode = 'favorite';
      this.updateAll();
      try {
        ...
        this.updateAll();
        this.logWarn(`keepFavoriteMode: 수동 복귀 실패 (다음 폴링에서 재시도): ${e.message || e}`);
      }
    } catch (e) {
  ...
  async call(method, args, silent = false) {
    ...
    // v2.2.2 — 성공 명령이 기본 레벨에서 안 보이던 것 해소: set_* 는 한국어 한 줄 info.
    // 입구에서 한 번만 남긴다(아래 재연결-재시도 경로가 있어도 같은 명령에 중복 출력 없음).
    const cmdLabel = fmtPurifier(method, args);
    if (cmdLabel) this.logInfo(cmdLabel);
    try {
