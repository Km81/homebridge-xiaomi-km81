 ...
 * 버전이다. aholstenson `miio` 패키지가 핸드셰이크/server-stamp 를 객체 수명 동안
 * 캐시해, 세션이 한 번 꼬이면(예: Mi Home 앱이 로컬 세션을 가져가면) 프로세스를
 * 재시작하기 전까지 복구되지 않는 문제가 있다. 본 프로토콜은 인스턴스를 새로 만들 때마다
 * 깨끗한 소켓으로 재핸드셰이크하므로 매 재연결이 fresh server-stamp 를 확보한다.
 * Qingping Air Monitor, 신형 miot 선풍기(xiaomi.fan.p45 등)처럼 miio 패키지를
 * 우회해야 하는 기기에서 사용.
 */
