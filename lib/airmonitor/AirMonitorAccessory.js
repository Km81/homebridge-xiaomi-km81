...
const { clamp, isFiniteNumber } = require('../common/helpers.js');
...
class AirMonitorAccessory {
  constructor(ctx, config) {
    ...
    this.initAccessory();
    // ⛔★임시 연결해제(v2.3.0): 폴링을 시작하지 않고, 소켓을 여는 monitor 도 만들지 않는다.
    //   lib/common/putAway.js (⚠️startPolling() 은 진입 시 _stopPolling 을 되돌리므로
    //   표식이 아니라 **호출하지 않는 것**이 잠금이다.)
    if (isPutAway(config)) {
  ...
  startPolling() {
    // setInterval 대신 자기 스케줄링 setTimeout 루프를 사용한다.
    // 느리거나 멈춘 pollOnce() 가 겹쳐 쌓이지 않도록, 현재 폴링이 끝난 뒤에만
    // 다음 폴링을 예약한다.
    ...
    const tick = async () => {
      try {
        ...
        if (!this._stopPolling) {
          this.pollTimer = setTimeout(tick, this.pollingIntervalMs);
        }
      }
    };
    tick();
  ...
  async pollOnce() {
    try {
      ...
      } else if (this.firstPollFailLogged) {
        // 가동 중 실패 경고가 찍힌 뒤 첫 성공 — 복구 명시 + 플래그 리셋
        // (리셋이 없으면 다음 실패 스트릭의 warn이 영구히 debug로 강등되는 버그, v2.0.0)
        this.firstPollFailLogged = false;
        this.log.info(`[${this.name}] 폴링 복구 ✓  ${log}`);
      } else {
