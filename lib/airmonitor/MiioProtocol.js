...
class MiioProtocol extends EventEmitter {
  ...
  createSocket() {
    ...
    this._socket.on('error', error => this.logger.debug(`(Protocol) Socket error: ${error}`));
  }

  destroySocket() { if (this._socket) this._socket.close(); }
  ...
  async send(address, method, params = [], options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const send = () => {
        this.handshake(address).catch(err => {
          if (err.code === 'timeout') {
